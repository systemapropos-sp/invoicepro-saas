...
export function EstimateForm() {
  ...
  const { currentBusiness, addEstimate, updateEstimate } = useBusiness();
  const isEditing = !!id;
  ...
  const [formData, setFormData] = useState({
    clientId: '',
    estimateNumber: `EST-${format(new Date(), 'yyyy')}-${String(currentBusiness.estimates.length + 1).padStart(4, '0')}`,
  ...
  useEffect(() => {
    if (isEditing && id) {
      const estimate = currentBusiness.estimates.find(e => e.id === Number(id));
      if (estimate) {
        setFormData({
          clientId: estimate.client_id.toString(),
          estimateNumber: estimate.estimate_number,
          issueDate: estimate.issue_date,
          validUntil: estimate.valid_until || format(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), 'yyyy-MM-dd'),
          notes: estimate.notes || '',
          terms: estimate.terms || '',
        });
        setTaxRate(estimate.tax_rate);
        setDiscountAmount(estimate.discount_amount);
      }
    }
  }, [id, currentBusiness.estimates, isEditing]);

  const handleItemChange = (id: number, field: keyof EstimateItem, value: string | number) => {
  ...
  const handleSave = (send: boolean = false) => {
    ...
    if (isEditing) {
      updateEstimate(Number(id), estimateData);
      toast.success(send ? 'Estimate sent successfully' : 'Estimate saved as draft');
    } else {
      addEstimate(estimateData);
      toast.success(send ? 'Estimate created and sent' : 'Estimate saved as draft');
    }
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-bold text-[#1f1f1f]">{formData.estimateNumber}</h1>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="space-y-4">
                {services.length === 0 ? (
                  <p className="text-sm text-[#5d6c7b] text-center py-4">{t('estimates.addService')}</p>
                ) : (
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <p className="text-xs text-[#aaadb0] mt-1">Comments will appear at the bottom of your estimate</p>
              <textarea
        ...
        <div className="space-y-6">
          <Card>
            <CardContent className="p-6 space-y-4">
              <div>
                <Label className="text-sm text-[#5d6c7b]">Estimate Number</Label>
                <Input
                  value={formData.estimateNumber}
                  onChange={(e) => {
                    setFormData({ ...formData, estimateNumber: e.target.value });
                    setHasChanges(true);
                  }}
                  className="mt-1"
                />
              ...
              <div>
                <Label className="text-sm text-[#5d6c7b]">Valid Until</Label>
                <Input
                  type="date"
